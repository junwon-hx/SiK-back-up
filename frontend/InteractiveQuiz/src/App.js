...
function App() {
  ...
  const handleAnswer = useCallback(
    (answer) => {
      if (isOpenEnded && !answer.trim()) return;
  ...
  const sendToAirtable = useCallback(() => {
    // Build field object from question text → answer
    const fields = {};
    questions.forEach((q) => {
      fields[q.text] = answers[q.id] || "";
    });

    // Optionally store all answers as JSON too
    fields["FullData"] = JSON.stringify(answers);
    ...
    fetch("https://api.airtable.com/v0/appjOplqzGhygN7FZ/UserData", {
      ...
      headers: {
        Authorization:
          "Bearer pat04vfX2LDqLwwTy.b042e8c08345689f824e94562592833bc321ca1cde0187558aa318607b7315ba",
      ...
      .catch((err) => {
        ...
        alert("Airtable error. Check the console.");
      });
  }, [answers]);
